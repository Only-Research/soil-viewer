...
Soil Viewer is a way to view, edit and organize markdown files in folders. I made it because I work with AI agents on my Mac all day, and what we share is a tree of markdown files (the soil, I call it). I wanted one place to look at that tree, read what the agents wrote, and change it, without juggling windows. This is that place.

It's a personal tool, released as-is: Mac only, no accounts, no database, not built for teams. Claude Code wrote the code; I built the process around it, the commitments below, each with a test that fails if it's broken. A real programmer will find things to improve. The comments in the source say why everything is the way it is.

The folder tree *is* the state. A task's status is the folder it sits in; moving a card moves the file. There is no database and no metadata layer. The one thing the app remembers for itself is where you placed projects on the Projects board, kept in its own settings file and never in your folders. Everything else you do here you could have done in Finder, and anything you do in Finder shows up here.

It runs as a small server on your own machine and is used from a browser, desktop or phone.

## Design commitments

- **Your files come back exactly as you left them.** The editor holds the file's real text and paints formatting over it rather than parsing to a document model and writing back. Enforced in CI on every push to `main` and every pull request: a corpus of 68 documents (44 hand-written probes plus 24 sampled from a 1,500-file generated tree, so that every markdown construct in it is represented) round-trips **100% byte-identical** through both browser engines, checked against three deliberately lossy editors that must each be caught. Documentation-only commits skip CI, so the claim is about
...
may well be worth porting, since nothing in the architecture prevents it, but today it is a Mac
